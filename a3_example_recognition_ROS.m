...
% category = {'sweater'};%'pant','shirt','tshirt','sweater'
% category = 'tshirt';
size_class=3;
size_move=30;
...
% iter_k = 9;
	...
	% rangeMap = imresize(rangeMap, imresizeScale);
				...
				if sum(sum(dilatedI))>10400%10000
			...
			% if para.iscontinue  
			% 	merge_descriptors([current_dir,'/Features/local_descriptors_' category{iter_i} int2str(iter_j) '_move' int2str(iter_k) '.mat'], allfeatures_local, para, 'local');
			% 	merge_descriptors([current_dir,'/Features/global_descriptors_' category{iter_i} int2str(iter_j) '_move' int2str(iter_k) '.mat'], allfeatures_global, para, 'global');
			% else
